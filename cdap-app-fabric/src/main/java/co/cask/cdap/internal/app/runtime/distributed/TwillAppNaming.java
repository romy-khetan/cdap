...
/**
 * Utility class to convert to and from the Twill application name for a {@link ProgramId}.
 */
public class TwillAppNaming {
  ...
  static String toTwillAppName(ProgramId programId) {
    Id.Program program = programId.toId();
    return String.format("%s.%s.%s.%s", program.getType().name().toLowerCase(),
                         program.getNamespaceId(), program.getApplicationId(), program.getId());
  }

  /**
   * Given a Twill app name, returns the ProgramId that was used to construct this Twill app name.
   *
   * @throws IllegalArgumentException if the given app name does not match the {@link TwillAppNaming#APP_NAME_PATTERN}.
   */
  ...
  /**
   * Given a Twill app name, returns the ProgramId that was used to construct this Twill app name.
   * Returns null if mustMatch is false, and if the specified Twill app name does
   * not match the {@link TwillAppNaming#APP_NAME_PATTERN}.
