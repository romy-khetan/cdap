...
/**
 * Wraps user-defined implementation of {@link Partitioner} class which allows perform extra configuration.
 */
public class PartitionerWrapper extends Partitioner implements Configurable {

  private static final String ATTR_PARTITIONER_CLASS = "c.partitioner.class";
  ...
  public static void wrap(Job job) {
    if (WrapperUtil.setIfDefined(job, MRJobConfig.PARTITIONER_CLASS_ATTR, ATTR_PARTITIONER_CLASS)) {
  ...
  public void setConf(Configuration conf) {
    this.conf = conf;
    this.delegate = WrapperUtil.createDelegate(conf, ATTR_PARTITIONER_CLASS);
  }
