 ...
 *
 */
public class TwillAppNamingTest {

  @Test
  public void test() {
    ProgramId flowId = new NamespaceId("test_ns").app("my_app").flow("my_flow");
    String flowTwillAppName = TwillAppNaming.toTwillAppName(flowId);
    Assert.assertEquals("flow.test_ns.my_app.my_flow", flowTwillAppName);
    Assert.assertEquals(flowId, TwillAppNaming.fromTwillAppName(flowTwillAppName));

    // parsing from twill app name can be optional (return null)
    Assert.assertNull(TwillAppNaming.fromTwillAppName(Constants.Service.MASTER_SERVICES, false));

    try {
      // if passing true, throws exception, when parsing is not possible
      TwillAppNaming.fromTwillAppName(Constants.Service.MASTER_SERVICES, true);
